...
//! AIR** on the PREVRANDAO opcode (`0x44`).
//!
//! Mirrors `crate::evm_main_context_joint_prove`. Bytecode
//! `[0x44, 0x00]` (PREVRANDAO; STOP).
//!
//!   - The EVM main row at the PREVRANDAO step exposes the pushed
//!     u256 in `COL_OUTPUT0_L0..L3` and sets `COL_SEL_PREVRANDAO = 1`.
//!   - The block_context_readers_air row mirrors the same value
//!     bytes/limbs with `sel_prevrandao = 1`, `is_real = 1`.
//!
...
//! higher limbs via the 32-byte `prev_randao` decomposition.
//!
...
use crate::block_context_readers_air::{
    COL_SEL_PREVRANDAO as GADGET_COL_SEL_PREVRANDAO, COL_VALUE_LIMB_0, COL_VALUE_LIMB_1,
...
/// `(COL_OUTPUT0_L0..L3)` on PREVRANDAO rows to block_context_readers_air's
/// `(COL_VALUE_LIMB_0..L3)` on prevrandao-selector rows.
///
/// **Widened (task #211)**: previously a single-column `(L0 ↔ L0)` anchor;
/// now binds the full u256 via the cross-AIR LogUp β-RLC multi-column
/// path (`build_linkage_trace` β-encodes the 4 limbs into a single
/// tuple_column, verifier recomputes the same β-RLC from the per-AIR
/// openings). This closes the residual soundness gap where higher limbs
/// on either side could mismatch without detection.
pub fn make_evm_prevrandao_to_block_context_gadget_descriptor(
    evm_layer_index: usize,
    block_context_gadget_layer_index: usize,
) -> CrossAirLogUpDescriptor {
    CrossAirLogUpDescriptor {
        label: "evm_main_prevrandao_to_block_context_gadget_v2".into(),
        ...
        b_selector_column: Some(GADGET_COL_SEL_PREVRANDAO),
    }
}
...
mod tests {
    ...
    use crate::block_context_readers_air::{
        build_trace_polynomials as build_block_context_trace,
        BlockContextReadersConstraintSystem,
        BlockContextReadersWitness, PREVRANDAO_OPCODE,
    };
    ...
    use crate::trace::{COL_OUTPUT0_L1, COL_OUTPUT0_L2, COL_OUTPUT0_L3, COL_SEL_PREVRANDAO};
    use metavm_zkp::field::{CurveType, Scalar};
    use metavm_zkp::trace::TracePolynomials;

    fn prevrandao_stop_bytecode() -> Vec<u8> {
        vec![0x44, 0x00]
    }

    fn build_evm_main_trace(curve: CurveType) -> TracePolynomials {
        let bytecode = prevrandao_stop_bytecode();
        let cols = execute_bytecode(&bytecode, &[])
            .expect("execute_bytecode must succeed for PREVRANDAO;STOP");
        TracePolynomials::from_vm_trace(&cols, curve)
    }

    fn extract_prevrandao_value(
        evm_polys: &TracePolynomials,
    ) -> Result<[u8; 32], &'static str> {
        let sel_col = &evm_polys.columns[COL_SEL_PREVRANDAO].evaluations;
        ...
        Err("EVM main trace must contain at least one PREVRANDAO row")
    }
    ...
    fn build_block_context_gadget_trace(
        ...
        let value = extract_prevrandao_value(evm_polys)
            .expect("EVM main trace must contain at least one PREVRANDAO row");
        let w = BlockContextReadersWitness::from_events(&[(PREVRANDAO_OPCODE, 0u64, value)]);
        build_block_context_trace(&w, curve)
    }

    #[test]
    fn descriptor_consistency() {
        let d = make_evm_prevrandao_to_block_context_gadget_descriptor(0, 1);
        assert_eq!(d.label, "evm_main_prevrandao_to_block_context_gadget_v2");
        ...
        assert_eq!(d.a_selector_column, Some(COL_SEL_PREVRANDAO));
        assert_eq!(d.b_selector_column, Some(GADGET_COL_SEL_PREVRANDAO));
        ...
        let evm_row = evm_row.expect("EVM main trace must contain at least one PREVRANDAO row");
        // Widened (task #211): assert ALL 4 limbs match between EVM main
        // and the block_context gadget — not just L0.
        for k in 0..4 {
        ...
        let gadget_sel = &ctx_polys.columns[GADGET_COL_SEL_PREVRANDAO].evaluations;
        ...
        let linkages = vec![make_evm_prevrandao_to_block_context_gadget_descriptor(0, 1)];
    ...
    #[test]
    #[ignore = "slow: joint_prove + joint_verify EVM main + block_context_readers_air PREVRANDAO (BLS48-581, ~30-60s release)"]
    fn honest_joint_verify_true() {
        ...
        let (proofs, ext) = joint_prove(&traces, &linkages, &scheme)
            .expect("honest 2-AIR joint_prove (EVM main + block_context PREVRANDAO) must succeed");
