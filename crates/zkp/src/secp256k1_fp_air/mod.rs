//! Limb-level AIR for **secp256k1** non-native `Fp` arithmetic.
//!
//! Mirrors [`crate::nonnative_fp_air`] (BLS12-381 base Fp) but specialised
//! to the real secp256k1 prime
//!
//! ```text
//! p = 2^256 - 2^32 - 977
...
//! ```
//!
//! All field elements are 256 bits, held as **4 big-endian u64 limbs**.
//!
//! # Why a separate module
//!
//! The existing `nonnative_fp_air` is hard-coded to a 381-bit modulus with
//! `LIMBS_PER_FP = 6`. Scalar-mul rows in the secp256k1 group descriptors
//! (Round 25 task #184) currently feed a BLS12-381 Fp witness as a
//! placeholder; the algebraic soundness story for ECDSA recovery needs the
//! Fp witness to live over the real secp256k1 base field. This module
//! provides that AIR; once it exists, the per-iteration block populator in
//! `secp256k1_group_descriptors` can route Mul rows here.
//!
//! # Row shape (one row = one Fp op)
//!
//! Limb columns are big-endian (index 0 = MS u64). Carry/borrow columns
//! use LSB-first indexing (index 0 = carry out of the LS limb).
//!
//! ```text
...
//!
//! -- Add aux --
//! 12      add_unred_sum      4    a + b unreduced (BE)
//! 16      add_carries        4    LSB-first carry chain
//! 20      add_reduce_flag    1
//! 21      add_trial_diff     4    unred_sum - p (BE)
//! 25      add_sub_borrows    4    LSB-first borrow chain for trial sub
//!
//! -- Sub aux --
//! 29      sub_unred_diff     4    a - b unreduced (BE)
//! 33      sub_borrows        4    LSB-first borrow chain
//! 37      sub_add_back_flag  1
//! 38      sub_trial_sum      4    unred_diff + p (BE)
//! 42      sub_add_carries    4    LSB-first carry chain for trial add
//!
//! -- Mul aux (Barrett-style q·p + r = a·b) --
//! 46      prod_limbs         8    8-limb product a·b (BE)
//! 54      quotient_limbs     4    q such that a·b = q·p + r (BE)
//! 58      qp_limbs           8    8-limb product q·p (BE)
//! 66      mul_carry_ab       8    carry chain for a·b schoolbook (LSB-first, ~66-bit)
//! 74      mul_carry_qp       8    carry chain for q·p schoolbook (LSB-first)
//! 82      mul_sum_carry      8    binary carry chain for qp + r_padded = prod
//!
//! -- Slack (c < p) --
//! 90      slack_limbs        4    p - 1 - c (BE)
//! 94      slack_borrows      4    LSB-first borrow chain
//!
//! -- Selectors --
//! 98      sel_add            1
//! 99      sel_sub            1
...
//! # Constraints
//!
//! Identical algebraic shape to `nonnative_fp_air`'s 19 constraint
//! categories, only with `LIMBS_PER_FP = 4` and `LIMBS_PER_PROD = 8`.
//! Inv reuses the entire Mul body (host-side witness `b = a^{-1}`,
//! `c = 1`) and only adds a `c == 1` constraint.
//!
//! # Scope (this initial drop)
//!
//! - Column layout, witness population, per-row constraint evaluation
//!   (`evaluate_constraints`), and lookup declarations.
//! - All 4 ops (Add / Sub / Mul / Inv).
//! - Range-check decls at 64-bit (limbs) / 72-bit (mul carries) / 1-bit
//!   (binary flags).
//! - Not yet implemented: full `VmConstraintSystem` integration, prove /
//!   verify hook-up, descriptor wiring to overhaul the BLS12-381
//!   placeholder Fp witness in `populate_scalar_mul_iter_block_trace`
//!   (deferred to a follow-up task once the per-iteration block columns
//!   are refactored to carry secp256k1-shaped operand windows).
...
/// secp256k1 base-field prime `p = 2^256 - 2^32 - 977`, big-endian limbs.
...
pub const NUM_SECP256K1_FP_COLUMNS: usize = COL_SEL_INV + 1;
...
/// secp256k1 base-field element. Big-endian 4-limb representation; reduced
/// modulo `P_LIMBS` whenever it's the output of an op constructor.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct Fp { pub limbs: [u64; 4] }
...
fn shift_left_4_into_8(m: &[u64; 4], s: usize) -> [u64; 8] {
    // Treat m as a 256-bit number; the result is at most 256 + s ≤ 512 bits,
    // fits in 8 limbs as long as s ≤ 256.
    // Build (m << s) into 8 BE limbs via explicit u64 limb shifting.
    // Convert m to LE: m_le[k] = m[3 - k] for k in 0..4.
...
pub struct AddWitness {
    pub sum: [u64; 4],
    pub add_carries: [u64; 5],     // [..4]: limb-out, [4]: final overflow
...
pub fn alloc_trace(num_rows: usize, curve: CurveType) -> Vec<Vec<Scalar>> {
    ...
    (0..NUM_SECP256K1_FP_COLUMNS).map(|_| vec![zero.clone(); num_rows]).collect()
}

// ─── Schoolbook for the AIR's exact 4×4 → 8 chain ────────────────────────

/// Column-by-column 4×4 schoolbook returning LSB-first product + LSB-first
/// carry chain (matches the AIR constraint shape).
...
/// Compute `qp + r_padded = prod` 8-limb add; return LSB-first sum+carries.
...
/// Witness `p - 1 - r` (BE) with borrow chain LSB-first; caller verifies
/// `borrows[3] == 0` (i.e. r < p).
...
pub fn populate_row(
    ...
    assert_eq!(columns.len(), NUM_SECP256K1_FP_COLUMNS, "columns shape");
    let zero = Scalar::zero(curve);
...
pub fn evaluate_constraints(columns: &[&Vec<Scalar>], beta: &Scalar) -> Vec<ConstraintEval> {
    assert_eq!(columns.len(), NUM_SECP256K1_FP_COLUMNS, "expected {} columns", NUM_SECP256K1_FP_COLUMNS);
...
mod tests {
    ...
    #[test]
    fn secp256k1_fp_air_column_layout_is_consistent() {
        ...
        assert_eq!(COL_ADD_UNRED_SUM_OFFSET, 12);
        assert_eq!(COL_ADD_CARRIES_OFFSET, 16);
        assert_eq!(COL_ADD_REDUCE_FLAG, 20);
        assert_eq!(COL_ADD_TRIAL_DIFF_OFFSET, 21);
        assert_eq!(COL_ADD_SUB_BORROWS_OFFSET, 25);
        assert_eq!(COL_SUB_UNRED_DIFF_OFFSET, 29);
        assert_eq!(COL_SUB_BORROWS_OFFSET, 33);
        assert_eq!(COL_SUB_ADD_BACK_FLAG, 37);
        assert_eq!(COL_SUB_TRIAL_SUM_OFFSET, 38);
        assert_eq!(COL_SUB_ADD_CARRIES_OFFSET, 42);
        assert_eq!(COL_PROD_OFFSET, 46);
        assert_eq!(COL_QUOTIENT_OFFSET, 54);
        assert_eq!(COL_QP_OFFSET, 58);
        assert_eq!(COL_MUL_CARRY_AB_OFFSET, 66);
        assert_eq!(COL_MUL_CARRY_QP_OFFSET, 74);
        assert_eq!(COL_MUL_SUM_CARRY_OFFSET, 82);
        assert_eq!(COL_SLACK_OFFSET, 90);
        assert_eq!(COL_SLACK_BORROWS_OFFSET, 94);
        ...
        assert_eq!(NUM_SECP256K1_FP_COLUMNS, 102);
    }

    #[test]
    fn secp256k1_fp_air_known_add_vector() {
    ...
    #[test]
    fn secp256k1_fp_air_known_mul_vector() {
        // Spot-check a small known multiplication: 7 * 11 = 77 (no reduction).
        let a = Fp::from_u64(7);
        ...
        // 2^128 * 2^128 wraps via the modulus.
        let two128 = Fp { limbs: [0, 1, 0, 0] };
        let prod = two128.mul(&two128);
        // 2^256 mod p = p + (2^256 - p) - p = 2^32 + 977 (since p = 2^256 - 2^32 - 977).
        let expected = Fp { limbs: [0, 0, 0, (1u64 << 32) + 977] };
        assert_eq!(prod, expected, "2^256 mod p must equal 2^32 + 977");
    ...
    #[test]
    fn secp256k1_fp_air_known_inv_vector() {
        // Spot-check: 2 * 2^{-1} = 1 mod p.
    ...
    #[test]
    fn secp256k1_fp_air_mixed_ops_in_one_trace() {
        let a = Fp::from_u64(7);
        let b = Fp::from_u64(11);
    ...
    #[test]
    fn secp256k1_fp_air_rejects_tampered_c_limb() {
    ...
    #[test]
    fn secp256k1_fp_air_rejects_double_selector() {
    ...
    #[test]
    fn secp256k1_fp_air_lookup_declarations_are_well_formed() {
        let decls = lookup_declarations();
        assert!(!decls.is_empty());
        for d in &decls {
            assert!(d.column_index < NUM_SECP256K1_FP_COLUMNS,
    ...
    #[test]
    fn secp256k1_fp_air_padding_row_is_valid() {
    ...
    #[test]
    fn secp256k1_fp_air_descriptor_wiring_smoke() {
        // Descriptor-wiring smoke: assert that the host-side Fp::mul output
        // for sample (a, b) pairs lands in `c_limb` of the populated row, and
        // that those rows individually satisfy every constraint category.
        // This is the minimal contract the secp256k1_group_descriptors
        // populate_scalar_mul_iter_block_trace populator needs to honor when
        // it routes Mul rows here instead of nonnative_fp_air.
        let test_pairs = vec![
            (Fp::from_u64(2), Fp::from_u64(3)),
            (Fp::from_u64(0xdead_beef), Fp::from_u64(0xcafe_babe)),
            (Fp { limbs: [0, 0, 1, 0] }, Fp { limbs: [0, 0, 0, 0xffff_ffff] }),
        ];
        let curve = CurveType::Bls48581;
        for (a, b) in &test_pairs {
            let expected = a.mul(b);
            let columns = populate_trace(&[FpOp::Mul { a: *a, b: *b }], curve, None);
            for i in 0..4 {
                let got = columns[c_limb(i)][0].to_u64();
                assert_eq!(got, expected.limbs[i],
                    "c_limb[{}] mismatch for a={:?} b={:?} expected={:?}", i, a, b, expected);
            }
            let beta = beta_challenge();
            let refs = col_refs(&columns);
            let evals = evaluate_constraints(&refs, &beta);
