//! `extra_data` sub-tree pair AIR.
//!
//! Per-row exposes the single `sha256_pair(packed_root, length_chunk)
//! -> root` invocation from an [`ExtraDataHtrWitness`] (the
//! mix_in_length step for `extra_data: ByteList[32]`).
//!
//! # Soundness scope
//!
//! - Per-row `(input_64, output_32)` matches a `Sha256Extract` row.
//! - HASH at the root invocation = CLAIMED_ROOT.
//! - CLAIMED_ROOT exposed for payload-pair binding.
...
pub const ROOT_INVOCATION_INDEX: usize = 0;
...
pub fn build_trace_polynomials(
    witness: &ExtraDataHtrWitness,
    curve: CurveType,
) -> TracePolynomials {
    let invocations = vec![witness.invocation];
    build_trace_polynomials_from_invocations(&invocations, curve)
}
...
pub struct ExtraDataPairConstraintSystem {
    pub num_rows: usize,
    pub omega: Option<Scalar>,
    pub domain_size: Option<u64>,
}

impl ExtraDataPairConstraintSystem {
...
impl VmConstraintSystem for ExtraDataPairConstraintSystem {
    ...
    fn lookup_declarations(&self) -> LookupRequirements {
        ...
        for k in 0..32 {
            for (name, off) in [
                ...
                declarations.push((
                    LookupDeclaration {
                        label: format!("extra_data_pair_{}_{}_8bit", name, k),
                        column_index: off + k,
                        max_bits: 8,
...
pub fn make_extra_data_pair_to_sha256_extract_linkage_descriptor(
    extra_data_pair_layer_index: usize,
    ...
    crate::cross_air_logup::CrossAirLogUpDescriptor {
        label: "extra_data_pair_sha256_extract_v1".into(),
        a_layer_index: extra_data_pair_layer_index,
        a_columns,
...
/// Bind extra_data-pair's CLAIMED_ROOT (computed extra_data merkle
/// root) ↔ payload-pair's CLAIMED_EXTRA_DATA_ROOT (the extra_data
/// field root claim). Closes the 2nd of 2 sub-tree soundness gaps.
pub fn make_extra_data_pair_to_payload_pair_linkage_descriptor(
    extra_data_pair_layer_index: usize,
    ...
    for k in 0..32 { b_columns.push(payload::COL_CLAIMED_EXTRA_DATA_ROOT_OFFSET + k); }

    crate::cross_air_logup::CrossAirLogUpDescriptor {
        label: "extra_data_pair_root_to_payload_pair_claimed_extra_data_v1".into(),
        a_layer_index: extra_data_pair_layer_index,
        a_columns,
        a_selector_column: Some(COL_IS_ROOT_BOUND_AT),
        b_layer_index: payload_pair_layer_index,
        b_columns,
        b_selector_column: Some(payload::COL_IS_EXTRA_DATA_LEAF_BOUND_AT),
    }
}
...
mod tests {
    ...
    fn sample() -> ExtraDataHtrWitness {
        ExtraDataHtrWitness::from_extra_data(vec![0xDE, 0xAD, 0xBE, 0xEF])
    }

    #[test]
    fn trace_populates_1_real_row() {
        ...
        assert_eq!(trace.columns[COL_IS_ROOT_BOUND_AT].evaluations[0].to_u64(), 1);
    }

    #[test]
    fn claimed_root_matches_witness_root() {
        let w = sample();
        let trace = build_trace_polynomials(&w, CurveType::Bls48581);
        for k in 0..32 {
            assert_eq!(
                trace.columns[COL_CLAIMED_ROOT_OFFSET + k].evaluations[0].to_u64(),
                w.root[k] as u64,
            );
        }
    }
    ...
    fn payload_linkage_descriptor_shape() {
        let d = make_extra_data_pair_to_payload_pair_linkage_descriptor(0, 1);
        assert_eq!(d.a_columns.len(), 32);
        assert_eq!(d.b_columns.len(), 32);
        assert_eq!(d.label, "extra_data_pair_root_to_payload_pair_claimed_extra_data_v1");
        assert_eq!(d.a_selector_column, Some(COL_IS_ROOT_BOUND_AT));
        use crate::execution_payload_pair_air as payload;
        assert_eq!(d.b_selector_column, Some(payload::COL_IS_EXTRA_DATA_LEAF_BOUND_AT));
    }
    ...
    fn standalone_prove_with_scheme_passes() {
        ...
        let cs = ExtraDataPairConstraintSystem::new(trace.num_rows)
