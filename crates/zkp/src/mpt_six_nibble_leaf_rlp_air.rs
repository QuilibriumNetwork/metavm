//! MPT 6-nibble even-length-path leaf gadget (Phase 7 of #92).
//!
//! Companion to the Phase 1 (64-nibble), Phase 3 (4-nibble), and Phase 6
//! (3-nibble odd) leaf gadgets. Same structural pattern as Phase 3
//! (short-form list, even-length path, 32-byte value, HP prefix 0x20)
//! but with 6 path nibbles → 3 path bytes → 39-byte total RLP.
//!
//! **Demonstrates the post-helper-refactor pattern**: this entire
//! gadget is ~250 lines of business logic, vs ~700 for Phase 3 before
//! the [`crate::mpt_rlp_gadget_helpers`] extraction. The shared
//! helpers carry the bulk of the body-evaluation arithmetic.
//!
//! # Canonical 39-byte RLP layout
//!
//! ```text
...
//!   [6]      0xa0   value-string header (= 0x80 + 32)
//!   [7..39]  ─      32 value bytes
//!   [39..256]       zero-pad to MAX_RLP_LEN
//! ```
...
pub const NUM_PATH_NIBBLES: usize = 6;
pub const NUM_PATH_BYTES: usize = NUM_PATH_NIBBLES / 2; // 3
pub const NUM_VALUE_BYTES: usize = 32;
pub const KEY_PATH_BYTE_WIDTH: usize = 32;

pub const HP_LEN: usize = 1 + NUM_PATH_BYTES; // 4
pub const HP_STRING_HEADER: u64 = 0x80 + HP_LEN as u64; // 0x84
pub const HP_PREFIX_BYTE: u64 = 0x20;
pub const VALUE_STRING_HEADER: u64 = 0x80 + NUM_VALUE_BYTES as u64; // 0xa0
pub const RLP_PAYLOAD_LEN: u64 = (1 + HP_LEN + 1 + NUM_VALUE_BYTES) as u64; // 38
pub const RLP_LIST_HEADER: u64 = 0xc0 + RLP_PAYLOAD_LEN; // 0xe6
pub const RLP_OUTPUT_LEN: usize = 1 + RLP_PAYLOAD_LEN as usize; // 39
...
const RLP_OFFSET_VALUE_HEADER: usize = RLP_OFFSET_PATH_BYTES + NUM_PATH_BYTES; // 6
const RLP_OFFSET_VALUE_BYTES: usize = RLP_OFFSET_VALUE_HEADER + 1; // 7
...
pub fn make_mpt_six_nibble_leaf_rlp_linkage_descriptor(
    ...
    crate::cross_air_logup::CrossAirLogUpDescriptor {
        label: "mpt_six_nibble_leaf_rlp_v1".into(),
        a_layer_index: mpt_layer_index,
        a_columns,
        a_selector_column: Some(mpt_col::IS_PHASE7_LEAF_6N_SHAPE),
...
mod tests {
    ...
    fn sample_path() -> [u8; NUM_PATH_BYTES] { [0x12, 0xab, 0x34] }
    ...
    fn descriptor_well_formed() {
        use crate::mpt_air::col as mpt_col;
        let desc = make_mpt_six_nibble_leaf_rlp_linkage_descriptor(0, 1);
        assert_eq!(desc.label, "mpt_six_nibble_leaf_rlp_v1");
    ...
    fn mpt_phase7_selector_fires_on_3_byte_key_single_leaf() {
        ...
        assert_eq!(rows[0].key_path_bytes[..3], key[..]);
        for k in 3..32 {
    ...
    /// Phase 7 gadget for a real 3-byte-key single-leaf trie proof.
    /// Mirror of `joint_prove_mpt_short_leaf_rlp_linkage` (Phase 3),
    /// `joint_prove_mpt_extension_rlp_linkage` (Phase 4), and
    /// `joint_prove_mpt_odd_leaf_rlp_linkage` (Phase 6).
    #[test]
    #[ignore = "slow: full joint_prove + joint_verify across 2 AIRs; \
                run with --release --ignored"]
    fn joint_prove_mpt_six_nibble_leaf_rlp_linkage() {
        ...
        let linkage = make_mpt_six_nibble_leaf_rlp_linkage_descriptor(0, 1);
        let traces: Vec<(
        ...
        let (proofs, ext_proof) = joint_prove(&traces, &linkages, &scheme).expect(
            "joint_prove must succeed for honest MPT + Phase 7 gadget",
        );
        ...
        assert!(
            valid,
            "joint verifier must accept honest single-leaf Phase 7 proof"
        );
    }

    /// Phase 7 cross-AIR LogUp tampering regression: corrupt MPT-side
    /// `KEY_PATH_BYTE[0]` on the Phase 7 leaf row while leaving the
    /// gadget honest. Multiset diverges, `joint_prove` rejects fast.
    /// Parallel to the Phase 1+2/3/6 tampering regressions.
    #[test]
    #[ignore = "slow: full joint_prove with tampered witness; \
                run with --release --ignored"]
    fn joint_prove_mpt_six_nibble_leaf_rlp_rejects_tampered_key_path() {
        ...
        // Tamper KEY_PATH_BYTE[0] (honest 0x12 → 0xff). The cross-AIR
        // LogUp 322-tuple includes KEY_PATH_BYTE on the MPT side; the
        // gadget side stays honest at 0x12. Multiset mismatches.
        ...
        assert!(
            result.is_err(),
            "joint_prove must reject tampered KEY_PATH_BYTE on Phase 7 row"
        );
    }

    #[test]
    #[ignore = "slow: full prove + verify; run with --release --ignored"]
    fn mpt_six_nibble_leaf_rlp_proof_round_trips() {
        use crate::scheme::bls48581_scheme::Bls48581Scheme;
        use crate::scheme::CommitmentScheme;
        let curve = CurveType::Bls48581;
        let scheme = Bls48581Scheme::new();
